...
export function ExerciseCard({
  ...
  // Handle both new + old schema safely
  const primaryMuscle =
    exercise?.primaryMuscle ||
    exercise?.muscleGroups?.[0] ||       // fallback for old data
    "";
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute left-0 top-0 bottom-0 bg-blue-600 rounded-full transition-all duration-300 ease-in-out ${isSelected ? "opacity-100 w-1.5" : "opacity-0 w-0"
          }`}
      />

      {/* Content wrapper that shifts when selected */}
      <div
        className={`flex items-center gap-3 flex-1 transition-all duration-300 ease-in-out ${isSelected ? "pl-4" : "pl-0"
          }`}
      >
        {/* Circular Image/GIF on Left */}
        <div className="relative flex-shrink-0 size-15 rounded-full overflow-hidden bg-gray-100">
          {mediaUrl && !imageError ? (
            <>
              {imageLoading && (
                <div className="absolute inset-0 flex items-center justify-center bg-gray-200">
                  <Dumbbell className="size-5 text-gray-400 animate-pulse" />
                </div>
              )}
              <img
                src={mediaUrl}
        ...
        <div className="flex-1 min-w-0">
          ...
          {primaryMuscleGroup && (
            <div className="flex items-center gap-3 mt-0.5">
              ...
              {isCustom && (
                <span className="px-3 py-0.5 text-sm rounded-[10px] border border-blue-100 bg-black text-white font-medium">
                  Custom
                </span>
              )}
            </div>
        ...
        >
          <div className="size-6 border border-black rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition-colors">
