...
export default function SelectOtherMuscle() {
  ...
  const handleUpdate = () => {
    // convert to array and send to parent / API
    const selectedArray = Array.from(selected);
    // TODO: replace with API call or navigate back with selection
    alert("Selected muscles: " + selectedArray.join(", "));
    // example: router.back();
  };
  ...
  return (
    <div className="bg-white text-gray-900 relative pb-28">
        ...
        {filtered.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Right side: chevron + checkbox (checkbox appears as circle that toggles to blue with check) */}
              <div className="flex items-center gap-4">

                <span
                  className={`flex items-center justify-center w-8 h-8 rounded-[5px] border ${
                    isChecked ? "bg-blue-600 border-transparent" : "bg-white border-gray-200"
                  }`}
                  aria-hidden
                >
                  {isChecked ? <Check className="size-7 text-white" /> : <div className="w-4 h-4" />}
                </span>
              </div>
            </button>
          );
      ...
      {/* Bottom fixed update button (only show when any selected) */}
      {selected.size > 0 && (
